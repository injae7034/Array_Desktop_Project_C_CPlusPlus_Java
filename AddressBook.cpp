...
{
	//<주의>AddressBook 소멸자가 호출되면 그 멤버들도 같이 다 소멸된다.
	//Array<Personal> personals는 AddressBook의 멤버이기 때문에 같이 소멸되는데 
	//이때 Array라이브러리의 소멸자가 호출되서 Array가 다 할당해제가 되기 때문에 
	//AddressBook의 소멸자에는 아무 내용이 없어도 된다.
}
